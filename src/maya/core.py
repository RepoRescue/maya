    ...
    def datetime(self, to_timezone=None, naive=False):
        ...
        if to_timezone:
            ...
            try:
                ...
                dt = Datetime.fromtimestamp(self._epoch, timezone.utc)
            except: # Fallback for before year 1970 issue
                from datetime import timezone
                dt = Datetime.fromtimestamp(0, timezone.utc) + timedelta(microseconds=self._epoch*1000000)
    ...
    def slang_date(self, locale="en"):
        ...
        Keyword Arguments:
            locale -- locale to translate to, e.g. 'fr' for french.
                       (default: 'en' - English)
        """
        # Convert to local timezone for humanize.naturaldate
        # humanize compares dates in local timezone, not UTC
        import datetime
        dt_local = self.datetime().astimezone()

        try:
            return _translate(dt_local, locale)
        except KeyError:
            pass
...
def when(string, timezone="UTC", prefer_dates_from="current_period"):
    ...
    """
    # For relative time expressions like "midnight", we need special handling
    # to ensure we get the current day's midnight when it's past midnight
    if string.lower() in ['midnight', 'noon']:
        import pytz
        ...
        # Parse with 'current_period' first
        settings = {
            "TIMEZONE": timezone,
        ...
        if dt is not None:
            # Check if the result is in the past relative to now
            dt_in_tz = dt.astimezone(tz)
            if dt_in_tz.date() < now_in_tz.date():
                # If it's a past date, try with 'future' preference
...
def parse(string, timezone="UTC", day_first=False, year_first=True, strict=False):
    ...
    try:
        ...
        # Fallback to datetime.fromisoformat for formats pendulum doesn't support
        from datetime import datetime as Datetime
        import re

        string_str = str(string)

        # Handle special ISO 8601 format: 2016T14 -> 2016-01-01T14:00:00
        if re.match(r'^\d{4}T\d{1,2}$', string_str):
            year = int(string_str[:4])
            hour = int(string_str[5:])
            dt = Datetime(year, 1, 1, hour, 0, 0, tzinfo=pytz.timezone(timezone))
        else:
            try:
                dt = Datetime.fromisoformat(string_str)
            except Exception:
                raise
